...
void set_p_ville(struct ville* ptr){
...
int resetYcentrales(struct centrale* centrales){
...
int add_centrale(struct centrale** centrale, int id){
  ...
  if(get_centrale(*centrale, id)!=NULL){
    return -2;
  };
  temp->id = id;
  temp->y = -1;
...
int listCentrales(char* filename, struct centrale* centrales){
  ...
  while(centrales != NULL){
    ...
    if(lignes != NULL){
      while(lignes != NULL){
        fprintf(fp, "D%d:L%d:V%d:P%d\n", centrales->id, lignes->id, lignes->ville->code, lignes->puissance);
...
// TODO Missing the ID parameter
int add_ligne(struct centrale* centrale, int puissance, struct ville* ville){
  if (ville == NULL || centrale == NULL){
    logmsg("Issue with argument : NULL value\n");
    return -1;
  }
